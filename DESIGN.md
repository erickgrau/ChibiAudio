# DESIGN — Onyx Dark + Soft Glass

Visual direction for ChibiAudio. **Blend A (Onyx Dark) + B (Soft Glass materials).**  
Plus paywall and Bandcamp browser follow the same tokens.
...
- Empty state when no library yet (point to Add Folder)
- Mini player chrome matches Wave 1

### Now Playing
- Huge hero (~52% viewport height), minimal chrome
- **Visualizer suite** on the hero (swipe or glass picker); transport unchanged
...
- Lyrics sheet (`.ultraThinMaterial` / soft glass): synced auto-scroll, unsynced text, or empty state
- AirPlay: `AVRoutePickerView` on transport row
- Add to Playlist sheet from Now Playing
...
| `LocalMusic/Views/HomeView.swift` | Home tab |
...
| `LocalMusic/Views/RadioView.swift` | Browse / Search / Favorites |
...
1. Home (default) — empty library + Add Folder  
...
- Paywall sheet: Onyx canvas + Soft Glass cards, amber subscribe CTA, Restore Purchases
- Free-tier ad banner sits above the mini player on browse tabs only — never Now Playing / DAC / visualizer
- Visualizer picker shows locked Plus modes with paywall entry
